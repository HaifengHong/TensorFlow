...
# 设定每个批次的大小，每次训练用batch_size个样本，矩阵形式
batch_size = 100 # batch_size取值越大，训练结果（准确率）越差

# 计算一共多少个批次
n_batch = mnist.train.num_examples // batch_size # 整除

# 定义两个placeholder，注意格式tf.placeholder(dtype,shape=None,name=None)
...
# prediction = tf.nn.softmax(tf.matmul(x,W)+b) # 用softmax作激活函数。有误，不必在此tf.nn.softmax，因为tf.nn.softmax_cross_entropy_with_logits函数里已经封装了tf.nn.softmax功能
prediction = tf.matmul(x,W)+b

# 代价函数
# loss = tf.reduce_mean(tf.square(y-prediction)) # 此时的prediction = tf.nn.softmax(tf.matmul(x,W)+b)
...
# 结果存放在布尔型列表中
correct_prediction = tf.equal(tf.argmax(y,axis=1),tf.argmax(prediction,1)) # tf.argmax返回一维张量中最大值所在的位置,0按列/1按行；tf.equal返回True或False
...
with tf.Session() as sess:
    ...
    for epoch in range(21): # 把所有图片训练21次
        ...
        # print('Iter ' + str(epoch) + ',Testing Accuracy ' + str(acc)) # 用“+”表示字符串连接，必须将int转化成str
        print('Iter ', epoch,  ',Testing Accuracy ', acc) # 用“,”则不用转化
...
# prediction = tf.nn.softmax(tf.matmul(x,W)+b)情况下的结果：
# Iter 0,Testing Accuracy 0.9081
# Iter 1,Testing Accuracy 0.9158
# Iter 2,Testing Accuracy 0.9183
# Iter 3,Testing Accuracy 0.9205
# Iter 4,Testing Accuracy 0.9206
# Iter 5,Testing Accuracy 0.9226
# Iter 6,Testing Accuracy 0.9221
# Iter 7,Testing Accuracy 0.9227
# Iter 8,Testing Accuracy 0.9209
# Iter 9,Testing Accuracy 0.9232
# Iter 10,Testing Accuracy 0.9213
# Iter 11,Testing Accuracy 0.9241
# Iter 12,Testing Accuracy 0.9238
# Iter 13,Testing Accuracy 0.9235
# Iter 14,Testing Accuracy 0.9228
# Iter 15,Testing Accuracy 0.9215
# Iter 16,Testing Accuracy 0.9245
# Iter 17,Testing Accuracy 0.9251
# Iter 18,Testing Accuracy 0.9235
# Iter 19,Testing Accuracy 0.9256
# Iter 20,Testing Accuracy 0.9244

# prediction = tf.nn.softmax(tf.matmul(x,W)+b)情况下的结果：（与正确情况下差别不大）
